...
# Load environment variables (from auth folder)
auth_dir = Path(__file__).resolve().parent
load_dotenv(auth_dir / ".env.example")
load_dotenv(auth_dir / "app" / ".env", override=True)
...
try:
    ...
    print("2. Check username/password in app/.env file")
